...
from .models import User_Id,Camera_Id,UploadUrlAndImages,Status_Process,KnownLog,UnknownLog,UploadUrlFireDetection
import hashlib
...
def upload_imagesurl_and_url_face_detection(request):
    if request.method == 'POST':
        ...
        image_urls = image_urls.strip('][').split(',')
...
def get_images_save_encode(camera_id):
    try:
        ...
        input_folder_path = 'api/all_images'

        # Convert string representation of list to actual list
        image_url_list = image_urls.strip('[]').replace("'", "").split(', ')

        for image_url in image_url_list:
            parsed_url = urlparse(image_url)
            image_name = ((os.path.basename(parsed_url.path)).split('_')[0])+'.png'
            print(image_url)
            folder_path = os.path.join(input_folder_path, str(camera_id))
