...
from django.contrib import admin
from .models import User_Id, Camera_Id, UploadUrlAndImages, Status_Process, KnownLog, UnknownLog,Fire_Log,UploadUrlFireDetection,UploadMotionUrl,Motion_Log
...
class Status_ProcessAdmin(admin.ModelAdmin):
    list_display = ['process_id', 'camera_id', 'status','detection_type']
...
@admin.register(Motion_Log)
class Motion_LogAdmin(admin.ModelAdmin):
    list_display = ['detection_time', 'camera_id']

@admin.register(UploadMotionUrl)
class UploadUrlMotionDetectionAdmin (admin.ModelAdmin):
    list_display = [ 'camera_id','url']
